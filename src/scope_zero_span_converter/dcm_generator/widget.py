"""Formal DCM waveform-generator widget.

The GUI owns controls, presentation and file dialogs while waveform math and
serialization remain in the versionless algorithm modules.
"""
...
class DcmSwGeneratorWidget(QWidget):
    """Generate, save and restore repeatable DCM switching waveforms."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._build_ui()
        self.apply_parameters(DcmSwParameters(), schedule_generate=False)
        # Generate once synchronously.  A cancellable debounce timer is then the
        # only pending generation source, so loading a saved waveform can stop it
        # and guarantee that historical CSV samples remain on screen.
    ...
    def collect_parameters(self) -> DcmSwParameters:
        return DcmSwParameters(
            ...
            time_origin_s=self.time_origin_us.value() * 1e-6,
    ...
    def apply_parameters(
        ...
        *,
        ...
        try:
            ...
            self.time_origin_us.setValue(p.time_origin_s * 1e6)
    ...
    def _redraw(self, waveform: DcmSwWaveform) -> None:
        ...
        show_truth = self.show_truth_components_check.isChecked()
        ax1 = self.figure.add_subplot(211 if show_truth else 111)
    ...
    # Save / load
    # ------------------------------------------------------------------
    def _choose_parameters_path(self, csv_path: Path) -> Path | None:
        ...
        return Path(selected_json) if selected_json else None

    def load_saved_waveform(
        self,
        csv_path: str | Path,
        *,
        parameters_path: str | Path | None = None,
    ) -> DcmSwWaveform:
        """Restore one saved truth waveform without regenerating its samples."""
        ...
        self._auto_timer.stop()
        try:
            self.apply_parameters(waveform.parameters, schedule_generate=False)
            self._auto_timer.stop()
        finally:
        ...
        self.status_label.setText(
            "已恢复保存波形："
            f"{csv_path.name} | {waveform.points} 点 | "
    ...
    def load_waveform_dialog(self) -> None:
        ...
        parameters_path = self._choose_parameters_path(csv_path_obj)
        if parameters_path is None:
            return

        try:
            self.load_saved_waveform(
                csv_path_obj,
                parameters_path=parameters_path,
            )
        except Exception as exc:
