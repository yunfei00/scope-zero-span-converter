...
class ResearchWorkspaceWindow(QMainWindow):
    """Versionless waveform-research and batch-workspace foundation.

    The production :class:`main_window.MainWindow` overrides the conversion
    entry points with background-worker implementations.  The synchronous
    implementations here keep this focused workspace independently usable and
    preserve the historical v0.4 compatibility surface.
    """

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Scope Zero Span Converter - 波形研究")
        ...
        LOGGER.info("versionless waveform research workspace ready")
...
__all__ = ["ResearchWorkspaceWindow", "main"]


def main() -> int:
    app = QApplication(sys.argv)
    window = ResearchWorkspaceWindow()
    window.show()
    return app.exec()
